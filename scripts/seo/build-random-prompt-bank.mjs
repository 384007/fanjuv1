...
function buildEditorialBrief(profile) {
  ...
  const localDetails = rotateFrom(profile, "brief-local-details", isEn
    ...
    : [
        ...
        `这篇文章要把小桌饭局和大型线下聚会、相亲局、微信群拼饭区分开。`,
  ...
  const h2Roles = isEn
    ? [
        ...
        { role: "local details", instruction: `Write an H2 about a concrete local detail or tension specific to ${city} and ${topic}. Use this local detail as INSPIRATION only — do NOT copy it as a paragraph opening: "${localDetails[0]}". Rewrite the idea in your own words, anchored to a specific moment in ${city}. Must name a real local friction, not a generic "local tips" heading.` },
        { role: "trust criteria", instruction: `Write an H2 that names one concrete signal — about the listing, venue, cost, or guest mix — that a first-timer in ${city} would use to decide whether this specific ${topic} table is worth showing up for. Angle: "${lens}". FORBIDDEN openers: never start with "Judging", "Judge", "How to choose", "What to look for", "Host reliability", "Venue clarity", "Trust", or any generic quality-assessment phrase. The heading must describe a city-specific moment or friction, not a reusable evaluation checklist.` },
    ...
    : [
        ...
        { role: "local details", instruction: `写一个 H2，关于${city}和${topicDisplayZh}饭局特有的本地细节或张力。以下本地细节仅供参考，禁止直接复制为段落开头：「${localDetails[0]}」。用自己的语言重新表达这个细节，锚定在${city}的某个具体时刻。必须点名真实的本地摩擦，不能是通用的"本地攻略"标题。` },
...
function userPromptFor(profile, editorialBrief) {
  ...
  return [
    ...
    `输出契约：第一个字符必须是「#」；H1 自然包含「饭局 / 饭局app / Fanju饭局 / 城市+主题饭局」之一；第一段第一句必须同时出现「${profile.cityNameLocalized}」「${profile.topicNameLocalized}」和「饭局app」或「Fanju饭局」；第一段前 220 字内必须同时包含城市名「${profile.cityNameLocalized}」、主题名「${profile.topicNameLocalized}」、「饭局app」，以及精确短语「不是相亲保证」「不是随机群聊」「不是无限刷资料」（不能改写，缺少任何一个得 0 分）；正文 12-14 个自然段，每个 H2 下正好两个自然段；无重复段落开头；无公开链接；无 JSON。正文中必须出现「不适合」「先别报名」「不该报名」「最好不要」「不建议」之一，缺少则 0 分。`,
