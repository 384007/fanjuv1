...
function userPromptFor(profile) {
  ...
  if (isEn) {
    return [
      ...
      `Body requirements: 4,200-6,200 characters; at least 12 natural paragraphs; no filler. Use blank lines between paragraphs. The body must start with a unique H1 line beginning "# "; the H1 must include ${profile.cityNameLocalized}, mention Fanju app naturally, and must not be a reusable guide title or a generic style label. Start the public article with an answer-summary paragraph in the first 120 words explaining that Fanju app is a social dining app for small, clearly described meals and real-world connections. Then include a short key-points bullet list covering who it suits, the core dinner scenario, and comfort or trust cues, but do not put a heading above that list. After that, write 5-7 H2 sections with original city/topic-specific headings shaped by the angle. Avoid checklist-style section labels; every H2 must sound like an editor wrote it for this exact city, topic, audience, and tension. Across the article, naturally resolve the reader's decision points before joining: local fit, table rhythm, host and venue quality, guest mix, comfort boundaries, skip signals, and a concrete next move. Fold those points into original sections instead of using them as literal H2 labels. Every H2 must contain at least two separate paragraphs with distinct ideas. Use H3 headings only for concrete objections or FAQ-style questions. If the body has fewer than 10 public paragraphs, it will be rejected.`,
  ...
  return [
    ...
    `正文要求：3,800-5,800 字符；至少 12 个自然段；段落之间必须空行；不要灌水。正文必须以独特 H1 开头，也就是第一行以「# 」开头；H1 必须包含「${profile.cityNameLocalized}」并自然出现「饭局app」，不能是可复用的指南标题，也不能是风格标签。开头必须是 answer-summary 式自然段，在前 200 字解释饭局app / Fanju 是围绕小桌吃饭、清晰主题和线下连接的社交应用。随后直接写一个简短要点列表，覆盖合适人群、饭局场景和安全重点，但不要给这个列表单独加 H2/H3 标题。然后写 5-7 个 H2，小节标题必须围绕${profile.cityNameLocalized}、${profile.topicNameLocalized}和本次角度重新拟定。不要用清单式通用小标题；每个 H2 都要像为这座城市、这个主题、这类读者和一个具体张力写出的编辑标题。全文要自然回答读者报名前会想清楚的决定点：本地适配、这一桌的节奏、主理人/餐厅/同桌质量、舒适边界、哪些信号说明不该去、下一步怎么做。把这些内容揉进原创小节，不要把这些决定点原样当标题。每个 H2 下至少写 2 个扎实、互不重复的段落，每段约 170-260 个汉字。H3 只用于具体疑问或 FAQ 式问题。少于 10 个公开自然段会被拒绝。`,
