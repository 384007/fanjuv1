...
function userPromptFor(profile, editorialBrief) {
  ...
  return [
    ...
    `输出契约：第一个字符必须是「#」；H1 自然包含「饭局 / 饭局app / Fanju饭局 / 城市+主题饭局」之一；第一段第一句必须同时出现「${profile.cityNameLocalized}」「${profile.topicNameLocalized}」和「饭局app」或「Fanju饭局」；第一段必须逐字包含「不是相亲保证」「不是随机群聊」「不是无限刷资料」这三个精确短语（不能改写）；正文 12-14 个自然段，每个 H2 下正好两个自然段；无重复段落开头；无公开链接；无 JSON。正文中必须出现「不适合」「先别报名」「不该报名」「最好不要」「不建议」之一，缺少则 0 分。`,
