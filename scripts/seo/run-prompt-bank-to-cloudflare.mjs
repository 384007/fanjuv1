...
function retryPrompt(basePrompt, attempt, issues) {
  ...
  return [
    ...
    isEn
      ...
      : `质量重试 ${attempt}：上一稿未通过这些类别：${issueSummary}。请根据 editorial brief 从头重写，并一次满足自动质量门。只返回文章正文，第一行必须以「# 」开头。标题/H1 必须自然包含中文城市名和「饭局 / 饭局app / Fanju饭局 / 城市+主题饭局」之一；第一段第一句必须包含城市、主题和「饭局app」或「Fanju饭局」。首段 120-220 字必须解释饭局app / Fanju饭局是什么，落到城市和主题，并包含精确短语「不是相亲保证」「不是随机群聊」「不是无限刷资料」。必须正好写 6 个独立「## 」标题；H3 只在真实需要时使用；禁止 H4-H10。每个 H2 都要为这个城市、主题、角度、人群和一个具体本地张力重写，不要把 brief 问题原文当标题。【禁用 H2 模式】以下任何形式的 H2 都会导致 0 分重写，必须完全避免：以「适合谁」「不适合谁」「核心饭局场景」「安全边界」「退出」「安全重点」「边界」「一桌饭」「主理人信号」「舒适边界」「下一步行动」「如何判断」「结语」开头的标题，或任何可以套用到其他城市的通用句式；H2 标题末尾禁止追加「，回到XXX饭局」等导航后缀。每个 H2 必须锚定在「城市名+主题词」的具体场景，且只适用于这篇文章。【不适合谁硬性要求】正文中必须包含「不适合」「先别报名」「不该报名」「最好不要」「不建议」之一，缺少则 0 分。【本地细节硬性要求】至少 5 个完整句子同时包含「城市名」和以下任意一个词：街区、跨区、到场、离场、费用、主理人、同桌、公共场所、本地、同城；缺少则扣分。【英文噪音硬规则】正文、标题、H2 中禁止出现任何英文词汇（如 faq、meetup、jazz、miles、davis、chet、baker、brt 等），必须全部替换为对应中文。${lengthGuidance} 严禁重复：每个自然段的开头句必须与其他所有段落的开头句完全不同；正文中不得出现与首段内容相同或高度相似的段落；每个 H2 下的内容必须覆盖全新角度，不得重述已有段落的核心句。不要摘要，要更具体、更本地、更完整；不要反复使用同一句式开头。不要包含 JSON、YAML frontmatter、代码块、Markdown 链接、裸 URL、href 或 HTML a 标签。`
  ].join("\n")
}
