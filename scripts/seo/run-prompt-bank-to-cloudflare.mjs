...
function retryPrompt(basePrompt, attempt, issues) {
  ...
  return [
    ...
    isEn
      ...
      : `质量重试 ${attempt}：上一稿未通过这些类别：${issueSummary}。请根据 editorial brief 从头重写，并一次满足自动质量门。只返回文章正文，第一行必须以「# 」开头。标题/H1 必须自然包含中文城市名和「饭局 / 饭局app / Fanju饭局 / 城市+主题饭局」之一；第一段第一句必须包含城市、主题和「饭局app」或「Fanju饭局」。首段 120-220 字必须解释饭局app / Fanju饭局是什么，落到城市和主题，并包含精确短语「不是相亲保证」「不是随机群聊」「不是无限刷资料」。必须正好写 6 个独立「## 」标题；H3 只在真实需要时使用；禁止 H4-H10。每个 H2 都要为这个城市、主题、角度、人群和一个具体本地张力重写，不要把 brief 问题原文当标题。至少包含 5 个本地细节、3 个真实疑问、2 个判断标准、1 个不适合谁、1 个安全或退出边界。${lengthGuidance} 严禁重复：每个自然段的开头句必须与其他所有段落的开头句完全不同；正文中不得出现与首段内容相同或高度相似的段落；每个 H2 下的内容必须覆盖全新角度，不得重述已有段落的核心句。不要摘要，要更具体、更本地、更完整；不要反复使用同一句式开头。不要包含 JSON、YAML frontmatter、代码块、Markdown 链接、裸 URL、href 或 HTML a 标签。`
  ].join("\n")
}
